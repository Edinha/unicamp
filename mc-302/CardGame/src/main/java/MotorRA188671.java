...
public class MotorRA188671 extends Motor {
	...
	protected void processarJogada(Jogada umaJogada) throws LamaException {
		switch (umaJogada.getTipo()) {
			case ATAQUE:
				...
				// alvo morreu ou ficou com quanto de vida? Trate o caso do herói como alvo também.
				break;
			case LACAIO:
	...
	private class JogadorInfo {
		...
		JogadorInfo(Integer jogador, Integer vida, List<Carta> mao, Baralho baralho, List<Carta> lacaiosMesa, Integer mana, Jogador referencia) {
			...
			this.lacaiosMesa = (List<CartaLacaio>) UnoptimizedDeepCopy.copy(lacaiosMesa);

			this.name = "Heroi " + jogador;
			this.danoFadiga = 1;
		...
		void fadiga() {
			...
			imprimir("Fadiga: O " + this.name + " recebeu dano por falta de cartas. (Vida atual: " + this.vida + ")");
		}
