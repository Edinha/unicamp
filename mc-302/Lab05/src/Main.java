	...
	private static void multiplasPartidas(Baralho baralho1, Baralho baralho2, int verbosidade, int tempoLimite, int n){
		...
		for(int p = 0; p < 2; p++){
			for(int w = 0; w < n/2; w++){
				...
				Motor partida;
				if(moeda)
					partida = new Motor(deck1, deck2, Mao1, Mao2, jogA, jogB, verbosidade, tempoLimite, "classe jogA", "classe Jog188671");
				else
					partida = new Motor(deck2, deck1, Mao2, Mao1, jogB, jogA, verbosidade, tempoLimite, "classe Jog188671", "classe jobB");
				...
				if( (moeda && vencedor) || (!moeda && !vencedor) ){
					...
					System.out.println("O jogador 188671 venceu! (classe "+jogB.getClass().getName()+"), que é o "+ (moeda?"segundo":"primeiro") +" jogador desta partida.");
					b++;
				}
			}
		...
		System.out.println("Jog188671 wins: "+b);
		return;
	}
}
